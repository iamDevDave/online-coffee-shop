...
class TCPDF {
	 ...
	 * @param boolean $cafetx if true reset the X position on direction change.
	 * @public
	 * @since 2.0.000 (2008-01-03)
	 */
	public function setRTL($enable, $cafetx=true) {
		$enable = $enable ? true : false;
		$cafetx = ($cafetx AND ($enable != $this->rtl));
		$this->rtl = $enable;
		$this->tmprtl = false;
		if ($cafetx) {
			$this->Ln(0);
	 ...
	 * @param boolean $cafetmargins if true reset left, right, top margins and Y position.
	 * @public
	 * @since 2.1.000 (2008-01-07)
	 * @see getPage(), lastpage(), getNumPages()
	 */
	public function setPage($pnum, $cafetmargins=false) {
	 ...
	 */
	public function lastPage($cafetmargins=false) {
		$this->setPage($this->getNumPages(), $cafetmargins);
	}
	 ...
	 * @param boolean $cafeth if true reset the last cell height (default true).
	 * @param int $stretch font stretch mode: <ul><li>0 = disabled</li><li>1 = horizontal scaling only if text is larger than cell width</li><li>2 = forced horizontal scaling to fit cell width</li><li>3 = character spacing only if text is larger than cell width</li><li>4 = forced character spacing to fit cell width</li></ul> General font stretching and scaling values will be preserved when possible.
	 ...
	 */
	public function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $ln=1, $x=null, $y=null, $cafeth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0, $valign='T', $fitcell=false) {
		$prev_cell_margin = $this->cell_margin;
		$prev_cell_padding = $this->cell_padding;
		...
		$this->setCellMargins(0, 0, 0, 0);
		if (TCPDF_STATIC::empty_string($this->lasth) OR $cafeth) {
			// reset row height
			$this->resetLastH();
		}
		if (!TCPDF_STATIC::empty_string($y)) {
		...
		if (($h > 0) AND $this->inPageBody() AND (($y + $h + $mc_margin['T'] + $mc_margin['B']) > $this->PageBreakTrigger)) {
			...
			$cafth = ($h - $newh); // cell to be printed on the next page/column
			$h = $newh;
		}
		...
		if ($ishtml) { // ******* Write HTML text
			$this->writeHTML($txt, true, false, $cafeth, true, $align);
			$nl = 1;
			...
			if ($maxh > 0) {
				// get text height
				$text_height = $this->getStringHeight($w, $txt, $cafeth, $autopadding, $mc_padding, $border);
	 ...
	 * @param float $w Width of cells. If 0, they extend up to the right margin of the page.
	 * @param boolean $cafeth if true reset the last cell height (default false).
	 ...
	 * @since 4.5.011
	 */
	public function getNumLines($txt, $w=0, $cafeth=false, $autopadding=true, $cellpadding=null, $border=0) {
		if ($txt === NULL) {
			return 0;
		}
		if ($txt === '') {
	 ...
	 *  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	 *  $pdf->MultiCell($w=0, $h=0, $txt, $border=1, $align='L', $fill=false, $ln=1, $x=null, $y=null, $cafeth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0);
	 *  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	 *  // get the new Y
	 *  $end_y = $pdf->GetY();
	 ...
	 * @public
	 */
	public function getStringHeight($w, $txt, $cafeth=false, $autopadding=true, $cellpadding=null, $border=0) {
		// adjust internal padding
		$prev_cell_padding = $this->cell_padding;
		$prev_lasth = $this->lasth;
		if (is_array($cellpadding)) {
		...
		$this->adjustCellPadding($border);
		$lines = $this->getNumLines($txt, $w, $cafeth, $autopadding, $cellpadding, $border);
	 ...
	 * @param mixed $cafize If true resize (reduce) the image to fit $w and $h (requires GD or ImageMagick library); if false do not resize; if 2 force resize in all cases (upscaling and downscaling).
	 ...
	 * @since 1.1
	 */
	public function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='', $align='', $cafize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		if ($this->state != 2) {
			return false;
		}
		if (TCPDF_STATIC::empty_string($x)) {
		...
		if (in_array($file, $this->imagekeys)) {
			...
			if (strpos($file, '__tcpdf_'.$this->file_id.'_imgmask_') === FALSE) {
				...
				if ((($oldsize < $newsize) AND ($cafize)) OR (($oldsize < $pixsize) AND (!$cafize))) {
					$newimage = true;
				}
			}
			...
			if (in_array($tempfile_plain, $this->imagekeys)) {
				...
				if ((($oldsize < $newsize) AND ($cafize)) OR (($oldsize < $pixsize) AND (!$cafize))) {
					...
					$newimage = false;
					// embed mask image
					$imgmask = $this->Image($tempfile_alpha, $x, $y, $w, $h, 'PNG', '', '', $cafize, $dpi, '', true, false);
					// embed image, masked with previously embedded mask
					return $this->Image($tempfile_plain, $x, $y, $w, $h, $type, $link, $align, $cafize, $dpi, $palign, false, $imgmask);
				}
			}
		}
		if ($newimage) {
			...
			$info = false;
			if ((method_exists('TCPDF_IMAGES', $mtd)) AND (!($cafize AND (function_exists($gdfunction) OR extension_loaded('imagick'))))) {
				...
				if (($ismask === false) AND ($imgmask === false) AND (strpos($file, '__tcpdf_'.$this->file_id.'_imgmask_') === FALSE)
					...
					return $this->ImagePngAlpha($file, $x, $y, $pixw, $pixh, $w, $h, 'PNG', $link, $align, $cafize, $dpi, $palign, $filehash);
				}
			}
	 ...
	 * @param mixed $link URL or identifier returned by AddLink().
	 * @param string $align Indicates the alignment of the pointer next to image insertion relative to image height. The value can be:<ul><li>T: top-right for LTR or top-left for RTL</li><li>M: middle-right for LTR or middle-left for RTL</li><li>B: bottom-right for LTR or bottom-left for RTL</li><li>N: next line</li></ul>
	 * @param boolean $cafize If true resize (reduce) the image to fit $w and $h (requires GD library).
	 * @param int $dpi dot-per-inch resolution used on resize
	 * @param string $palign Allows to center or align the image on the current line. Possible values are:<ul><li>L : left align</li><li>C : center</li><li>R : right align</li><li>'' : empty string : left for LTR or right for RTL</li></ul>
	 ...
	 * @see Image()
	 */
	protected function ImagePngAlpha($file, $x, $y, $wpx, $hpx, $w, $h, $type, $link, $align, $cafize, $dpi, $palign, $filehash='') {
		// create temp images
		if (empty($filehash)) {
			$filehash = md5($file);
		}
		...
		$imgmask = $this->Image($tempfile_alpha, $x, $y, $w, $h, 'PNG', '', '', $cafize, $dpi, '', true, false);
		// embed image, masked with previously embedded mask
		$this->Image($tempfile_plain, $x, $y, $w, $h, $type, $link, $align, $cafize, $dpi, $palign, false, $imgmask);
	}
	 ...
	 * @param bool $cafetx if true (default) reset the X position.
	 * @param boolean $rtloff if true always uses the page top-left corner as origin of axis.
	 * @public
	 * @since 1.0
	 * @see GetX(), GetY(), SetY(), SetXY()
	 */
	public function setY($y, $cafetx=true, $rtloff=false) {
		$y = floatval($y);
		if ($cafetx) {
			//reset x
			if (!$rtloff AND $this->rtl) {
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	 ...
	 * @public
	 */
	public function writeHTMLCell($w, $h, $x, $y, $html='', $border=0, $ln=0, $fill=false, $cafeth=true, $align='', $autopadding=true) {
		return $this->MultiCell($w, $h, $html, $border, $align, $fill, $ln, $x, $y, $cafeth, 0, true, $autopadding, 0, 'T', false);
	}
	 ...
	 * @public
	 */
	public function writeHTML($html, $ln=true, $fill=false, $cafeth=false, $cell=false, $align='') {
		$gvars = $this->getGraphicVars();
		// store current values
		$prev_cell_margin = $this->cell_margin;
		$prev_cell_padding = $this->cell_padding;
		...
		while ($key < $maxel) {
			...
			if ($dom[$key]['tag'] AND $dom[$key]['opening'] AND isset($dom[$key]['attribute']['nobr']) AND ($dom[$key]['attribute']['nobr'] == 'true')) {
				if (isset($dom[($dom[$key]['parent'])]['attribute']['nobr']) AND ($dom[($dom[$key]['parent'])]['attribute']['nobr'] == 'true')) {
					...
					$this_method_vars['reseth'] = $cafeth;
