...
class Order_model extends CI_Model {
    ...
    public function getOrderByUser($id) {
        $this->db->select('o_id, cafe_id, i_id, users.u_id, i_name, quantity, price, status, f_name, l_name, user_orders.date, users.email, users.phone,  success-date, username, address');
        $this->db->from('user_orders');
        $this->db->join('users', 'users.u_id = user_orders.u_id');
        $this->db->where('o_id', $id);
